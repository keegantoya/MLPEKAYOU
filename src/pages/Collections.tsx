      ...
      <main className="w-full max-w-none px-3 pb-4 pt-6 sm:px-6 sm:py-8">
        <section className={`rounded-[20px] border p-3.5 sm:rounded-[24px] sm:p-5 ${isLightMode
          ...
          <div className="flex items-end justify-between gap-4 px-1">
            <div>
              <p className={`text-xs font-semibold uppercase tracking-[0.12em] ${isLightMode ? "text-zinc-500" : "text-zinc-400"}`}>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:gap-3">
            {[
            { label: "Sets Mastered", value: `${completedSets}/${totalSets}` },
            {
                label: "Cards Collected",
                value: totalCardsCollected.toLocaleString(),
            },
        ].map((stat) => (<div key={stat.label} className={`rounded-xl border px-3 py-2.5 sm:rounded-2xl sm:px-4 sm:py-3 ${isLightMode
        ...
        <div className="mt-3 md:hidden">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            <button type="button" aria-pressed={hideMastered} onClick={() => setHideMastered((previous) => !previous)} className={`min-h-11 rounded-xl border px-3 text-sm font-semibold transition-colors ${hideMastered
            ? isLightMode
                ? "border-[#8a6a00]/35 bg-[#c89d13]/10 text-[#725700]"
                : "border-[#FFD54A]/60 bg-[#FFD54A]/10 text-[#FFE27A]"
            : isLightMode
                ? "border-black/10 bg-white text-zinc-600"
                : "border-white/10 bg-[#151718] text-zinc-300"}`}>
              {hideMastered ? "Show Mastered" : "Hide Mastered"}
            </button>
            <select aria-label="Sort collections" value={sortBy} onChange={(event) => setSortBy(event.target.value as "release" | "set")} className={`min-h-11 rounded-xl border px-3 text-base font-semibold outline-none ${isLightMode
          ...
          <div className="min-w-0 flex-1">
            {activeCategory === "" ? (<div className="py-20"/>) : filtered.length === 0 ? (<div className={`rounded-2xl border border-dashed p-8 text-center text-sm sm:p-10 sm:text-base ${isLightMode
                ...
                return (<div key={col.id} className="group relative">
                      ...
                      {isUnreleased && (<div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center">
                          <div className={`rounded-xl border px-4 py-2 text-center text-sm font-bold tracking-wide shadow-lg sm:text-base ${isLightMode
                            ? "border-[#8a6a00]/30 bg-white/90 text-[#725700]"
                            : "border-[#FFD54A]/40 bg-black/85 text-[#FFE27A]"}`}>
                            COMING SOON
                          </div>
                        </div>)}
                      {isHidden && !isUnreleased && !isWaiting && (<div className="pointer-events-none absolute inset-x-2 top-2 z-30">
                          <div className={`rounded-xl border px-3 py-2 text-xs font-semibold ${isLightMode
                      ...
                      {isMastered &&
                        !isHidden &&
                        !isUnreleased &&
                        !isWaiting && (<div className="pointer-events-none absolute left-2 top-2 z-30">
                            <div className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${isLightMode
                            ? "border-[#8a6a00]/20 bg-white/95 text-[#725700]"
