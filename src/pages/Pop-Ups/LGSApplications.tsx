...
const syncMobileLayout = () => setCompactMobile(media.matches);
    syncMobileLayout();
...
const input = `mt-1.5 w-full rounded-xl border px-3 py-2.5 text-base focus:outline-none focus:ring-2 focus:ring-[#b59525] sm:mt-2 sm:py-3 ${box}`;
const content = (
    <div className="p-4 sm:p-7">
      <header className={`sticky -top-4 z-20 -mx-4 -mt-4 mb-3 flex items-start justify-between gap-3 border-b px-4 py-3 sm:static sm:mx-0 sm:mt-0 sm:mb-6 sm:border-0 sm:px-0 sm:py-0 ${surface} ${light ? "border-black/10" : "border-white/10"}`}>
        <div>
          <p className={`text-xs font-semibold sm:text-sm ${light ? "text-[#765d12]" : "text-[#E7C84B]"}`}>Local game stores</p>
          <h1 id="lgs-application-title" className="mt-1 text-xl font-semibold leading-tight sm:text-2xl">LGS Hosting Organization Program</h1>
        </div>
        {onClose && <button type="button" onClick={onClose} aria-label="Close application" className={`shrink-0 rounded-xl border px-3 py-2 text-sm sm:text-base ${box}`}>Close</button>}
      </header>
      {loading ? <p role="status">Loading your application…</p> : loadFailed ? (
        ...
        <section className={`rounded-2xl border p-4 sm:p-5 ${box}`} role="status">
          ...
          <p className={`mt-2 break-words text-sm leading-6 sm:mt-3 sm:text-base sm:leading-7 ${muted}`}>{application.store_name} · Submitted {new Date(application.created_at).toLocaleDateString()}</p>
          <div className={`mt-3 space-y-3 text-sm leading-6 sm:mt-4 sm:space-y-4 sm:text-base sm:leading-7 ${muted}`}>
            {application.status === "pending" ? (
              <>
                <p>We’ve received your application, and it is awaiting review. You can return here to check its status.</p>
                <details open={!compactMobile} className={`rounded-xl border p-3 ${box}`}>
                  <summary className="cursor-pointer font-semibold">Review timing and contact details</summary>
        ...
        <form key={user.id} onSubmit={submit}>
          <p className={`mb-4 text-sm leading-6 sm:mb-6 sm:text-base sm:leading-7 ${muted}`}>Apply on behalf of your store to use MLPEKAYOU’s tournament tracking tools. Choose one authorized representative to apply. Each account may submit one application, and each store location should be represented only once.</p>
          <fieldset disabled={busy} className="space-y-4 disabled:opacity-70 sm:space-y-6">
            ...
            <section aria-labelledby="lgs-location-heading">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 sm:mt-4 sm:gap-4">
                ...
                ].map(([name, label, autoComplete, maxLength]) => <label key={name} className={`block text-sm font-medium sm:text-base ${name === "street_address" ? "col-span-2" : ""}`}>{label}<input name={String(name)} autoComplete={String(autoComplete)} maxLength={Number(maxLength)} required className={input} /></label>)}
              </div>
            </section>
            <section aria-labelledby="lgs-contact-heading">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 sm:mt-4 sm:gap-4">
                <label className="block text-sm font-medium sm:text-base">Your name<input name="contact_name" autoComplete="name" required maxLength={150} className={input} /></label>
                <label className="block text-sm font-medium sm:text-base">Your role at the store<input name="store_role" placeholder="Owner, manager, event organizer…" required maxLength={100} className={input} /></label>
                <label className="col-span-2 block text-sm font-medium sm:text-base">Contact email<input name="contact_email" type="email" autoComplete="email" defaultValue={user.email ?? ""} required maxLength={254} className={input} /></label>
              </div>
            </section>
            <fieldset><legend className="font-semibold">Has your store hosted Kayou events before?</legend>
              <div className="mt-2 grid grid-cols-2 gap-2 sm:mt-3 sm:gap-3">{[["yes", "We already run or have run events"], ["no", "We have never run an event"]].map(([value, label]) => <label key={value} className={`flex cursor-pointer items-start gap-2 rounded-xl border p-3 text-sm sm:gap-3 sm:p-4 sm:text-base ${box}`}><input type="radio" name="has_run_events" value={value} required className="mt-0.5 h-5 w-5 shrink-0 accent-[#b59525] sm:mt-1" /><span>{label}</span></label>)}</div>
            </fieldset>
            <fieldset aria-describedby="lgs-employee-help"><legend className="font-semibold">Do employees at your store already use MLPEKAYOU to track tournaments?</legend>
              ...
              <div className="mt-2 grid grid-cols-3 gap-2 sm:mt-3 sm:flex sm:flex-wrap sm:gap-3">{[["yes", "Yes"], ["no", "No"], ["unsure", "I’m not sure"]].map(([value, label]) => <label key={value} className={`flex cursor-pointer items-center justify-center gap-2 rounded-xl border p-2.5 text-center text-sm sm:justify-start sm:gap-3 sm:p-4 sm:text-base ${box}`}><input type="radio" name="employee_usage" value={value} required className="h-5 w-5 shrink-0 accent-[#b59525]" /><span>{label}</span></label>)}</div>
            </fieldset>
            <label className="block font-medium" htmlFor="lgs-additional-information">
              ...
              <textarea id="lgs-additional-information" name="additional_information" rows={compactMobile ? 3 : 5} maxLength={5000} aria-describedby="lgs-additional-help" className={`${input} resize-y`} />
            </label>
            <details open={!compactMobile} className={`rounded-2xl border p-4 sm:p-5 ${box}`}>
              <summary id="lgs-privacy-heading" className="cursor-pointer text-base font-semibold sm:text-lg">Privacy, accountability &amp; program information</summary>
  ...
  return onClose ? <dialog ref={dialog} aria-labelledby="lgs-application-title" onCancel={event => { event.preventDefault(); onClose(); }} className={`m-auto w-[calc(100%_-_1rem)] max-w-2xl overflow-y-auto overscroll-contain rounded-xl p-0 shadow-2xl backdrop:bg-black/70 sm:w-[calc(100%_-_2rem)] sm:rounded-2xl ${surface}`} style={{ colorScheme: light ? "light" : "dark", maxHeight: compactMobile ? "72dvh" : "90dvh" }}>{content}</dialog> : <main className={`min-h-screen px-3 py-4 sm:px-4 sm:py-6 ${surface}`} style={{ colorScheme: light ? "light" : "dark" }}><div className="mx-auto max-w-2xl">{content}</div></main>;
}
