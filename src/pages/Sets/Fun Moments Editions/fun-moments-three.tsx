...
const rarityNames: Record<string, string> = {
  ...
  HR: "HOLOGRAPHIC RARE",
  UR: "ULTRA RARE",
  LSR: "LIMITED SUPER RARE",
  SGR: "SUPER GOLDEN RARE",
  ZR: "ZENITH RARE",
  SC: "SECRET CARD",
  SZR: "SHINING ZENITH RARE",
};
            ...
            <div className="overflow-hidden border border-white/[0.09] bg-[#080b0b] shadow-[0_20px_55px_rgba(0,0,0,.5)]">
              <div className="border-b border-white/[0.07] bg-[#050707] px-4 py-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-[5px] uppercase tracking-[0.2em] text-[#FFD54A]/60">M3</span>
                </div>
              </div>
              ...
              <div className="relative border-b border-white/[0.07] p-4 sm:p-5">
                ...
                <p className="mt-4 font-mono text-[7px] uppercase leading-5 tracking-[0.08em] text-zinc-500">Click a card to mark it as owned. Your ISO list updates automatically based on the cards you still need.</p>
              </div>
                  ...
                  <section key={rarity} id={`rarity-${rarity}`} className="relative overflow-hidden border border-white/[0.08] bg-[#080b0b] p-3 shadow-[0_18px_45px_rgba(0,0,0,.42)] sm:p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-6">
                      {cards.filter((card) => card.rarity === rarity).map((card) => {
                        ...
                        const isWide = card.rarity === "SZR" && card.number === 1;

                        return (
                          <div
                            key={key}
                            className={`group relative cursor-pointer overflow-hidden rounded-md border border-white/[0.08] bg-[#050707] shadow-[0_10px_25px_rgba(0,0,0,.42)] transition-all duration-200 md:hover:z-20 md:hover:-translate-y-2 md:hover:border-[#FFD54A]/45 md:hover:shadow-[0_18px_38px_rgba(0,0,0,.58)] ${
                              isWide ? "col-span-2 aspect-[10/7]" : "aspect-[5/7]"
                            }`}
                          ...
                          >
                            <div className={`relative h-full w-full transform-style-preserve-3d transition-all duration-200 ${hoverEffects ? "md:group-hover:rotate-1 md:group-hover:scale-[1.035]" : ""} ${owned && !viewMode ? "rotate-y-180" : ""}`}>
                              <img
                                ...
                                className="absolute inset-0 h-full w-full scale-[1.10] object-cover object-center rounded-md backface-hidden"
                                alt=""
                              />
                              <img
              ...
              <div className="relative mx-auto w-full overflow-hidden rounded-md border border-[#FFD54A]/30 bg-[#050707] shadow-[0_22px_60px_rgba(0,0,0,.85)]" onClick={() => setZoomedCardFlipped(!zoomedCardFlipped)}>
                <div className="relative aspect-[5/7] w-full max-h-[66vh] overflow-hidden rounded-md">
                  <div className={`absolute inset-0 transform-style-preserve-3d transition-transform duration-500 ${zoomedCardFlipped ? "rotate-y-180" : ""}`}>
                    <img src={zoomedCard} className="absolute inset-0 h-full w-full scale-[1.10] object-cover object-center rounded-md backface-hidden" alt="" />
                    <img src={zoomedCardBack || ""} className="absolute inset-0 h-full w-full scale-[1.10] object-cover object-center rounded-md backface-hidden" style={{ transform: "rotateY(180deg)" }} alt="" />
                  </div>
                </div>
              </div>
            </TiltCard>
